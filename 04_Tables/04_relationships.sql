...
-- (Un usuario sólo puede tener un DNI. Un DNI sólo puede estar asociado a un usuario -> 1:1)
CREATE TABLE dni(
...
tabla B, pero un registro en la tabla B se relaciona con un sólo registro en la tabla A -> 1:n.
*/
...
-- El campo company_id de la tabla "users" es clave foránea de la clave primaria company_id de la tabla "companies" -> (Un empleado (usuario) sólo puede tener una empresa, pero una empresa puede tener muchos empleados (usuarios))
